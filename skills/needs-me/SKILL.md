...
1. **Preflight** (from the operator-kit skill), then collect only three kinds of row:
   ...
   - **Kiểm tra** — built and waiting for a person to look at the screen and confirm it is right. Work that is finished but unverified is not done, and a pile of it is the quietest way for a board to rot: it looks like progress and none of it has been seen. Group these into one row with a count when there are more than three, ranked by what reaches real users first — money, consent and anything already live before anything else — and point at `/proof` for each one.
...
4. **Sweep for stalls while you are here**: anything that has not moved in 12 hours joins the list as a `Gỡ kẹt` row with its age — measured from the last real status change, not from last-updated, which some boards re-stamp on a timer and which would hide a ticket stuck for weeks. The kit cannot push a message on its own, so this sweep is how a stall reaches a person.
5. **Cap the list at ten**, counting a grouped row as one — a Kiểm tra row standing for thirty items is one decision for the reader, not thirty. More than ten waiting means the queue itself is the problem: show the ten that block the most, say how many more there are, and say plainly that a queue this long needs a decision about the queue, not ten more decisions.
