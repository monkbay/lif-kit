...
3. **Is there anything to interpret?** Decide this before building anything.

   **Rõ ràng · Unambiguous and small** — a typo, exact wording they supplied, a colour or number they gave, showing or hiding something they named. **Do not build a page. Do not offer three readings.** Answer in chat in one line — `Trước: "Đăng Nhập" → Sau: "Đăng nhập"`, which screen it is on, the size — and file it in the fast lane. Then stop. Three pictures of a capital letter is how a tool gets abandoned in week one.

   **Có thể hiểu nhiều cách · Open to reading** — anything spatial ("sang phải", "chật"), anything that changes layout or flow, anything medium or larger, anything where two people could picture different results. That earns the full preview below.

   When it sits between the two, ask yourself whether you could be wrong about what they pictured. If you could, draw it. If you could not, just do it.

4. **Decide the kind.** Screen exists and misbehaves → a bug, under warranty, say so. Otherwise a change or a new thing. Never make the operator classify it.
5. **Establish the current state before drawing it.** Find the screen in the code, then capture it for real — see `references/honesty.md` in the operator-kit skill. If you could not capture it, label the frame `sketch — chưa kiểm chứng` and say why in one line.
6. **Check for a duplicate** on the board before creating anything. If the request already exists, reuse that ticket and say so.
7. **If they attached a picture**, it is the specification. Read it, build what it shows — colours, spacing and wording included — and say in one line what you took from it. Never substitute your own design taste, and never quietly change a detail you think is wrong: name it instead.
